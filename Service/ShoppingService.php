...
class ShoppingService
{
    ...
    public function getShippingForm(Order $Order)
    {
        ...
        // ADD FOR KR_AKI_CUSTOMIZER.
        $this -> addDeposit($builder);
        // ADD END FOR KR_AKI_CUSTOMIZER.
...
// ADD FOR KR_AKI_CUSTOMIZER.
  function addDeposit($builder) {
    // ADD FOR KR_AKI_CUSTOMIZER.
    $session = $this -> app["session"];
    $orderType = "";
    if ($session -> has(\Plugin\KrAkiCustomizer\Common\Constants::ORDER_DETAIL_ADDITIONAL_INFO_LIST)) {
      $orderDetailAdditionalInfoList = $session -> get(\Plugin\KrAkiCustomizer\Common\Constants::ORDER_DETAIL_ADDITIONAL_INFO_LIST);
    ...
    if ($orderType == "visit") {
      ...
      if ($depositable) {
        ...
        // 内金 or 前払い
        $builder -> add(
          \Plugin\KrAkiCustomizer\Common\Constants::PLG_PAY_METHOD,
          'choice',
          array(
            'required' => true,
            'label'  => "お支払い方法",
            'choices' => \Plugin\KrAkiCustomizer\Common\Utils::getPayMethod($settings),
            'constraints' => array(
